...
Python 3.x
Required libraries (install via pip):pip install pandas numpy scikit-learn xgboost joblib matplotlib seaborn
...
Pie Chart for Risk Level Distribution (risk_distribution.png):
...
Example (included below) illustrates the number of instances correctly classified (e.g., 41 high risk cases predicted as high risk) and misclassified (e.g., 4 low risk cases predicted as high risk).



Line Graph for Training and Validation Accuracy (accuracy_line_graph.png):
...
Example (included below) shows training accuracy reaching ~0.95 and validation accuracy stabilizing around 0.85-0.90 after 40 iterations.




Example Prediction
The script includes a function predict_maternal_risk to predict risk levels for new data. An example is provided with input values (Age=25, SystolicBP=130, DiastolicBP=80, BS=15, BodyTemp=98, HeartRate=86), and the predicted risk level is printed.
Saved Models
...
The line graph tracks accuracy by retraining the model at each iteration with a validation set. The default number of boosting rounds is 100, but this can be adjusted by modifying the n_estimators parameter in the XGBClassifier.
